...
export default function ProductCard({ product }: { product: Product }) {
  return (
    <div className="group relative flex h-full flex-col rounded-2xl border border-line bg-white p-2 transition-shadow hover:shadow-card">
      {/* Image + overlays */}
      <Link href={`/product/${product.id}`} className="relative block">
        ...
        <span className="absolute bottom-1 left-1 flex items-center gap-0.5 rounded-md bg-white/95 px-1.5 py-0.5 text-[10px] font-bold text-ink-soft shadow-sm backdrop-blur">
      ...
      {/* Info */}
      <Link href={`/product/${product.id}`} className="mt-2 block flex-1">
